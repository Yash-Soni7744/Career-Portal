...
def signup():
    if request.method == "GET":
        ...
        functions.send_notification(user_email, "Welcome to KRMU", "Welcome to KRMU's official app. We hope you have a great experience.")
        ...
        MONGODB_DB["users"].insert_one({
            ...
            "user_role": "student",
            "user_profile": {
                "user_name": MONGODB_DB["student_info"].find_one({"roll_no": user_email.split("@")[0]})["name"],
                "user_branch": MONGODB_DB["student_info"].find_one({"email": user_email})["branch"],
                "user_year": MONGODB_DB["student_info"].find_one({"email": user_email})["year"],
                "user_roll_no": MONGODB_DB["student_info"].find_one({"email": user_email})["roll_no"],
                "user_phone": MONGODB_DB["student_info"].find_one({"email": user_email})["phone"],
            },
            "user_notifications": [
                {
                    "notification_id": 1,
